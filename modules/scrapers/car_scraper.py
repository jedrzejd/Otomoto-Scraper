...
class CarScraper:
    ...
    def combine_data(self):
        ...
        xlsx_filenames = [os.path.join(self.data_directory, f"{model.strip()}.xlsx") for model in self.models]
        combined_data = []
        for filename in xlsx_filenames:
            try:
                combined_data.append(pd.read_excel(filename, index_col='Unnamed: 0'))
            except Exception:
                pass
